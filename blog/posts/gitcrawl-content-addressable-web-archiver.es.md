...
Aquí te echo el cuento de la arquitectura del motor en C99, cómo se conecta directo con la plomería interna de Git sin tocar el directorio de trabajo para ir volando, y los benchmarks de compresión.

---

## 1. Arquitectura del Pipeline

`gitcrawl` está pensado con la filosofía UNIX: una herramienta modular, escrita en C99 pulcro, sin dependencias raras ni runtimes pesados, lista para meterla en pipelines y cron jobs.
...
`gitcrawl` monta un parser de streams basado en el motor de [`unipaste`](https://github.com/riccivr/unipaste). Agarra el contenido semántico de verdad (`<main>`, `<article>`, o el bloque con más densidad de texto), convirtiendo títulos, tablas, listas y bloques de código a Markdown limpiecito.
...
3. **Archivos planos en disco / S3:** Guardar carpetas crudas produce condiciones de carrera (*Last Write Wins*), borrando versiones intermedias si dos agentes le caen a la misma URL casi al mismo tiempo. Además, te quedas sin compresión delta nativa.
...
Al guardar los snapshots en un repositorio Git estándar, puedes combinar `gitcrawl` con tus herramientas de diario en la terminal:
