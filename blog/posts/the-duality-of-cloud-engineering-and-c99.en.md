...
For most of my professional career, my day job has lived in the world of high-level web engineering and cloud infrastructure.

I started out writing PHP and building web applications with Laravel, handling relational databases, and shipping business logic. Over time, my work shifted into TypeScript, Node, AWS, serverless architectures, and event-driven microservices.

When I finish work and open a terminal to write code for myself, my brain wants something completely different. I build tools in C99, write POSIX stream filters, experiment with embedded hardware, and dig into binary formats.
...
In the cloud world, you orchestrate abstractions. You spend your day wiring AWS services, defining TypeScript interfaces, tuning API routes, and managing dependency trees. The goal is shipping reliable features quickly. If an API request responds in 80 milliseconds, that is considered good performance.
...
My resume, LinkedIn, and public professional footprint showed the pragmatic cloud engineer. I highlighted my experience with PHP, Laravel, TypeScript, SQL databases, and AWS pipelines.
...
When a recruiter or hiring manager looks for a Senior Full-Stack Engineer or Cloud Architect, they expect to see familiar buzzwords: React, Node, Laravel, Docker, Kubernetes, or AWS certifications.
...
- Will automated screening tools or non-technical recruiters get confused about my primary stack?
...
When a Node process runs out of memory in production, or a serverless worker suffers from socket exhaustion under high load, you cannot solve the issue by adding more abstractions. You need to understand how the operating system handles memory pages, network buffers, and I/O polling.
...
3. **Calm debugging.** When a high-level library fails with an obscure error, you can read the underlying C bindings or strace output without panic.
